...
<div class="min-h-screen">
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <!-- Stats Cards -->
        <div class="grid grid-cols-2 lg:grid-cols-10 gap-5">

            <!-- Monthly Spend -->
            <div class="stat-card col-span-2 lg:col-span-3 animate-slide-up" style="animation-delay:0.05s">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Monthly Spend</p>
                        <p class="text-3xl font-bold text-white">₱<?= number_format((float)$data['monthly_spend'], 2) ?></p>
                        <p class="text-gray-500 text-xs mt-1.5">≈ ₱<?= number_format((float)$data['annual_spend'], 2) ?>/yr</p>
                    </div>
            ...
            <a href="/subscriptions" class="stat-card col-span-2 lg:col-span-3 animate-slide-up hover:border-brand-500 hover:shadow-glow hover:-translate-y-0.5 transition-all duration-300 block cursor-pointer" style="animation-delay:0.1s">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-gray-400 text-xs font-medium uppercase tracking-wider mb-2">Active Subscriptions</p>
                        <p class="text-3xl font-bold text-white"><?= (int)$data['active_count'] ?></p>
                        <p class="text-gray-500 text-xs mt-1.5">Services active</p>
                    </div>
            ...
            <a href="/subscriptions" class="stat-card col-span-1 lg:col-span-2 p-4 animate-slide-up hover:border-amber-500 hover:shadow-glow hover:-translate-y-0.5 transition-all duration-300 block cursor-pointer" style="animation-delay:0.15s">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-gray-400 text-[10px] font-medium uppercase tracking-wider mb-1">Paused</p>
                        <p class="text-xl font-bold text-white"><?= (int)$data['paused_count'] ?></p>
                        <p class="text-gray-500 text-[10px] mt-0.5">Inactive</p>
                    </div>
                    <div class="stat-icon bg-amber-500/20 text-amber-400 !w-10 !h-10 !rounded-xl">
            ...
            <a href="/subscriptions" class="stat-card col-span-1 lg:col-span-2 p-4 animate-slide-up hover:border-red-500 hover:shadow-glow hover:-translate-y-0.5 transition-all duration-300 block cursor-pointer" style="animation-delay:0.2s">
                <div class="flex items-start justify-between">
                    <div>
                        <p class="text-gray-400 text-[10px] font-medium uppercase tracking-wider mb-1">Cancelled</p>
                        <p class="text-xl font-bold text-white"><?= (int)$data['cancelled_count'] ?></p>
                        <p class="text-gray-500 text-[10px] mt-0.5">Terminated</p>
                    </div>
                    <div class="stat-icon bg-red-500/20 text-red-400 !w-10 !h-10 !rounded-xl">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <?php else: ?>
                    <div class="space-y-3 max-h-72 overflow-y-auto pr-1 scrollbar-thin">
                        <?php foreach ($data['upcoming'] as $sub):
                            $daysUntil = (int) ceil((strtotime($sub['next_renewal_date']) - time()) / 86400);
                        ...
                        ?>
                            <a href="/subscriptions/show/<?= $sub['id'] ?>" class="flex items-center justify-between gap-3 p-3 rounded-xl bg-surface-600/50 border border-white/5 hover:border-brand-500/40 hover:-translate-y-0.5 transition-all duration-200 block cursor-pointer">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-white truncate"><?= htmlspecialchars($sub['service_name'], ENT_QUOTES) ?></p>
                                    <p class="text-xs text-gray-500"><?= date('M j, Y', strtotime($sub['next_renewal_date'])) ?></p>
                                </div>
